...
public class AccountDaoImplJUnit {
	...
	public void D_testUpdateAccount() {
		Account accountRead = accountDaoImpl
				.readAccountByEmail("test3@hotmail.com");
	...
	// Why does this test return AccountId=0 the first time it runs (notEqual)
	// and AccountId=1 the runs after that (equals).
	@Test
	public void E_testReadAccountByEmail() {
		LOG.info("Entering testReadAccountByEmail()...");
		Account readAccount = accountDaoImpl
				.readAccountByEmail("test3@hotmail.com");
		LOG.info("ReadingAccount1: " + readAccount.toString());
		Customer customer = new Customer(1, "Peter", "de", "Graaf");
		Account insertedAccount = new Account(customer, "test3@hotmail.com",
				"PasswordGewijzigd", 1);
